//! Feature resolution for crane projects.
//!
...
pub fn resolve_features(
    ...
    requested: &[String],
    use_defaults: bool,
) -> Result<BTreeSet<String>, CraneError> {
    ...
    while let Some(feat) = queue.pop_front() {
        ...
        if !all_features.contains_key(feat) {
            return Err(CraneError::ManifestParse(format!("unknown feature '{feat}'")));
        }
...
/// Convert active feature names to `-D` compiler flags.
/// `"tls"` → `"-DTLS"`, `"with_json"` → `"-DWITH_JSON"`.
/// Hyphens are normalised to underscores before uppercasing.
pub fn to_defines(features: &BTreeSet<String>) -> Vec<String> {
    features.iter()
        .map(|f| format!("-D{}", f.replace('-', "_").to_uppercase()))
        .collect()
}
...
mod tests {
    ...
    fn to_defines_uppercases() {
        ...
        defines.sort();
        assert_eq!(defines, vec!["-DTLS", "-DWITH_JSON"]);
    }
}
