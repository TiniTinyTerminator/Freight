...
/// Extract doc comments from source files and render an HTML reference site.
#[derive(Parser)]
#[command(name = "crane-doc", about = "Multi-language doc comment extractor and HTML renderer")]
struct Cli {
    ...
    /// Print a summary of extracted items without writing HTML
    #[arg(long)]
    dry_run: bool,
}
...
fn main() {
    ...
    if let Err(e) = render_html(&set, &cli.out) {
        eprintln!("error: failed to write docs: {e}");
        std::process::exit(1);
    }

    println!("✓ {total} items → {}", cli.out.join("index.html").display());
}
