...
fn sm_arch_h(v: &str, _: &str, _: &str, _: &str, _: &str) -> Result<Vec<String>, String> {
    if !v.is_empty() {
        Ok(vec![format!("--gpu-architecture={v}")])
    } else {
        Ok(vec![])
    }
}
...
fn lang_arch_h(v: &str, _: &str, arch: &str, _: &str, _: &str) -> Result<Vec<String>, String> {
    if !v.is_empty() && arch != v {
        Err(format!(
            "nvcc requires arch '{v}' but the effective target is '{arch}'"
        ))
    } else {
        Ok(vec![])
    }
...
pub fn nvcc() -> CompilerTemplate {
    TemplateDef {
        ...
        warning_flags: &[
            ...
            (
                "all",
                "-Xcompiler -Wall,-Wextra --Werror cross-execution-space-call,reorder",
            ),
            (
                "error",
                "-Xcompiler -Wall,-Wextra,-Werror --Werror all-warnings",
            ),
        ],
