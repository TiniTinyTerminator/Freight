...
pub fn ldc2() -> CompilerTemplate {
    TemplateDef {
        ...
        structure: &[
            ("include_dir", "-I{path}"),
            ("define", "-d-version={name}"),
            ("define_value", "-d-version={name}"),
            ("output", "-of={path}"),
            ("compile_only", "-c"),
            ("dep_file", ""),
            ("dep_file_mode", "none"),
    ...
    .build(
        &[("dip1000", dip1000_h as OptionHandlerFn, Some("false"))],
        &[],
    )
}
