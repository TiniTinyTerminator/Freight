...
fn try_load_cache() -> Option<Vec<PackageDoc>> {
    ...
    if cache.key != cache_key() { return None; }
    Some(cache.packages.into_iter().map(|p| PackageDoc {
        name: p.name, version: p.version, items: p.items, readme: None,
    }).collect())
}
...
fn save_cache(pkgs: &[PackageDoc]) {
    ...
    let cache = CacheFile {
        key: cache_key(),
        packages: pkgs.iter().map(|p| CachedPackage {
            name:    p.name.clone(),
...
fn load_or_scan(tx: &Sender<StdlibMsg>) -> Vec<PackageDoc> {
    ...
    if let Some(pkgs) = try_load_cache() {
        let _ = tx.send(StdlibMsg::Progress {
            done: 1, total: 1, label: "loaded from cache".to_string(),
        });
        return pkgs;
    }
    // Full scan.
...
fn cpp_stdlib(tx: &Sender<StdlibMsg>) -> Vec<PackageDoc> {
    ...
    for (i, dir) in dirs.iter().enumerate() {
        let label = dir.file_name()
            ...
            .unwrap_or("")
            .to_string();
        let _ = tx.send(StdlibMsg::Progress { done: i, total, label });
        if dir.is_dir() {
            items.extend(extract_dir(dir).items);
        }
    }
    if items.is_empty() { return Vec::new(); }
    vec![PackageDoc { name: "C++ Standard Library".to_string(), version: String::new(), items, readme: None }]
}

fn cpp_stdlib_dirs() -> Vec<PathBuf> {
    probe_cpp_include_paths().into_iter()
        .filter(|p| p.is_dir()
            && (p.join("bits").is_dir()
...
fn rust_stdlib(tx: &Sender<StdlibMsg>) -> Vec<PackageDoc> {
    let Some(src) = rust_stdlib_src() else { return Vec::new() };
    let libs = ["std", "core", "alloc"];
    let mut items: Vec<DocItem> = Vec::new();
    for (i, lib) in libs.iter().enumerate() {
        let _ = tx.send(StdlibMsg::Progress { done: i, total: libs.len(), label: lib.to_string() });
        let dir = src.join(lib).join("src");
        if dir.is_dir() { items.extend(extract_dir(&dir).items); }
    }
    if items.is_empty() { return Vec::new(); }
    vec![PackageDoc { name: "Rust Standard Library".to_string(), version: rust_version(), items, readme: None }]
}
